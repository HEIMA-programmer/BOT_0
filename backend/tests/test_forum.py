...
def test_posts_can_be_searched_by_title_or_tag(client, create_user):
    viewer = create_user(username='viewer', email='viewer@example.com')
    author = create_user(username='author', email='author@example.com')
    _create_post(author.id, 'Turnitin Guide', ForumPost.STATUS_APPROVED, zone='public')
    second = _create_post(author.id, 'Semester Reflection', ForumPost.STATUS_APPROVED, zone='public')
...
def test_admin_all_posts_includes_approved_friend_zone_posts(client, create_user):
    ...
    author = create_user(username='author', email='author@example.com')
    _create_post(author.id, 'Approved public post', ForumPost.STATUS_APPROVED, zone='public')
    _create_post(author.id, 'Approved friend post', ForumPost.STATUS_APPROVED, zone='friend')
...
def test_admin_cannot_pin_friend_zone_posts(client, create_user):
    ...
    author = create_user(username='author', email='author@example.com')
    friend_post = _create_post(author.id, 'Approved friend post', ForumPost.STATUS_APPROVED, zone='friend')
...
def test_friend_zone_pins_are_user_specific(client, create_user):
    ...
    friend_post = _create_post(author.id, 'Friend-only post', ForumPost.STATUS_APPROVED, zone='friend')
...
def test_regular_user_cannot_pin_invisible_friend_zone_post(client, create_user):
    ...
    friend_post = _create_post(author.id, 'Hidden friend post', ForumPost.STATUS_APPROVED, zone='friend')
    ...
    assert response.get_json()['error'] == 'Only approved friend zone posts visible to you can be pinned'
...
def test_friend_zone_excludes_forwards_from_non_friends(client, create_user):
    ...
    public_post = _create_post(owner.id, 'Owner public post', ForumPost.STATUS_APPROVED, zone='public')
...
def test_forward_can_target_friend_zone_and_is_visible_to_friends(client, create_user):
    ...
    public_post = _create_post(author.id, 'Public source post', ForumPost.STATUS_APPROVED, zone='public')
